# **Gradient Boost Tree (Cross Validation Split) in PySpark**
...
sc = SparkSession.builder.master('local[*]').appName('used_cars_data').getOrCreate()
file_df = sc.read.csv(file_location,header=True)
file_df.show()
...
paramGrid = ParamGridBuilder().addGrid(gbt.maxDepth, [2, 5]).addGrid(gbt.maxIter, [10, 20]).build()
...
gbtcv_evaluator = RegressionEvaluator(predictionCol="prediction",labelCol="price",metricName="r2")

print("R Squared (R2) on test data = %g" %gbtcv_evaluator.evaluate(prediction))

gbtcv_evaluator = RegressionEvaluator(labelCol="price", predictionCol="prediction", metricName="rmse")
